...
struct ConnectionsJobView: View {
            ...
            VStack {
              Spacer()
                ...
                Button(action:{
                    ...
                    if var user_connections = job_accounts.accounts_networks_job[cur_username] {
                        print("Account found")
                        
                        if var external_user_messages = user_connections[current_username_job] {
                            print("External Account found")
                            external_user_messages.append(connection_message)
                        } else {
                            print("External Account will be created")
                            user_connections[current_username_job] = [connection_message]
                        }
                        
                    } else {
                        print("Account being created")
                        job_accounts.accounts_networks_job[cur_username] = [current_username_job : [connection_message]]
                    }
                    print("Account Networks Job: \(job_accounts.accounts_networks_job)")
                    button_clicked = true
...
struct ConnectionsJobView_Previews: PreviewProvider {
